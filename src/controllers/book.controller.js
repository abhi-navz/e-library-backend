...
const { generateBookSummary } = require('../services/ai.service');
...
const getBookSummary = async (req, res, next) => {
  try {
    ...
    if (typeof book.aiSummary === 'string' && book.aiSummary.trim()) {
      return res.status(200).json({ bookId: book._id, summary: book.aiSummary, cached: true });
    }

    const summary = await generateBookSummary(book);
    // Do not overwrite a summary generated by another concurrent request.
    const updatedBook = await Book.findOneAndUpdate(
      { _id: book._id, $or: [{ aiSummary: null }, { aiSummary: '' }, { aiSummary: { $exists: false } }] },
      { $set: { aiSummary: summary } },
      { returnDocument: 'after' }
    );

    if (!updatedBook) {
      const cachedBook = await Book.findById(book._id);
      if (typeof cachedBook?.aiSummary === 'string' && cachedBook.aiSummary.trim()) {
        return res.status(200).json({ bookId: cachedBook._id, summary: cachedBook.aiSummary, cached: true });
      }
      throw createError('Unable to save the generated book summary.', 503);
    }

    return res.status(200).json({ bookId: updatedBook._id, summary: updatedBook.aiSummary, cached: false });
  } catch (error) {
    return handleBookError(error, next);
  }
};
...
const updateBook = async (req, res, next) => {
  try {
    ...
    if (['title', 'author', 'description', 'content'].some((field) => Object.hasOwn(req.body, field))) {
      book.aiSummary = null;
    }
    await book.save();
...
module.exports = { createBook, getBooks, getBookById, getBookSummary, updateBook, deleteBook };
